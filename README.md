...
1. Enable this plugin from the repo root (Cursor: `.cursor-plugin/`; Claude: `.claude-plugin/`; Codex: `.codex-plugin/`). This repo already points at itself:
   - Cursor: `.cursor/settings.json` (`plugins.stuc-stack.enabled`). Local load: `mkdir -p ~/.cursor/plugins/local && ln -sfn "$(pwd)" ~/.cursor/plugins/local/stuc-stack`, then reload and enable third-party plugins.
