...
`/stuc-mode` picks the playbook, routes Compose/Kotlin design through `stuc-chrisbanes` then chrisbanes skill **names**, Google workflows to android/skills **names**, unknown-path device exploration to `stuc-artemis`, and Android device proof to `android-verify`. This guide teaches that habit.
