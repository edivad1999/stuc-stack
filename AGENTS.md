...
Requires chrisbanes/skills, android/skills, and `android` on PATH. Those trees are not in this git repo. `.agents/skills/` is gitignored (install location). Plugin glue for chrisbanes is `stuc-chrisbanes`; the upstream router is `using-chrisbanes-skills`. Remaining enable steps: `.agents/stuc-stack-intake/design/install-status.md`.
