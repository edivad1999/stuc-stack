...
Kotlin and Android tests use the same check. `assertEquals("hello-world", slugify("Hello, World!"))` observes a result. `verify { repo.save(any()) }` with no asserted state does not.
