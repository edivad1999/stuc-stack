...
**Android devices.** Two agents must not share one emulator without a lock. One serial per **android-verify** run. Refuse double-drive. android-cli has `--device` and no lease. Artemis holds a per-device mutex on `mobile_run_task`. Treat a running Artemis task as that lock and do not Drive the same serial until it stops.
