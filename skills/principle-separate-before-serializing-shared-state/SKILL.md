...
**Android devices.** Two agents must not share one emulator without a lock. One serial per **android-verify** run. Refuse double-drive. android-cli has `--device` and no lease.
