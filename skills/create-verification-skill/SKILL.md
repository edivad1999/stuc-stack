...
- **Isolate:** can two instances run side by side (ports, data dirs, profiles, emulator serials)? If not, say so in the generated skill: refusing to double-drive a shared instance beats corrupting the user's session. Android: one emulator serial this run started. A running **stuc-artemis** task holds that serial until it stops.
