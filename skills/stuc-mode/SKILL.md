...
name: stuc-mode
description: >-
  stuc-stack agent style for concise replies, deliberate subagents, unslopped
  prose, simple Kotlin/Android code, and verified work. Use for /stuc-mode or
  requests to work in this stack's style.
disable-model-invocation: true
mode: true
icon: crown
color: yellow
reminder: New task? Playbook match or rigor needed -> apply /stuc-mode. Casual turn or user opts out -> don't.
---

# Stuc mode

Android/Kotlin overlays and git rules cite `P#`. Architecture taste: `references/android-opinions.md`.

## Non-negotiables

**Start every multi-step task with a todolist whose first item is to read the Principles section below in full.** Architecture and git taste: also read `references/android-opinions.md`. Follow the repo in front of you; do not invent a module graph. The principles ground every trigger here. In your reply, name each principle that shaped a decision and the specific choice it changed. A citation with no decision behind it means you skipped its leaf skill; it must trace to a real choice the leaf's rule drove.
...
- Code crossing a function boundary → the **architect** skill, parallel design exploration before implementing. Copy-the-last-screen / smallest-diff work may skip with `architect skipped: copy last similar screen`.
...
- Unknown-path Android exploration, cross-app repro, soak, or logcat hunt on a real device → **stuc-artemis** (MCP install / fail closed for that step only). Then prove with **android-verify**. An Artemis trace is not Done. (P18)
...
- **Prove It Works** (**principle-prove-it-works**). After a task, before declaring done. Verify against the real artifact, not a proxy or "it compiles". Android UI/device: **android-verify**. Name what ran. (P18)
...
**Use `subagent_type: "stuc-agent"` for any subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers). `/stuc-mode` and `stuc-agent` route through the same wrapper. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own `subagent_type` for diverse-model review; respect what the skill prescribes, don't override to `stuc-agent`.

**Defaults for every `Task` call.** `run_in_background: true`, agent mode (readonly strips MCP), file pointers not inlined context, explicit model per role (configurable via `/setup-stuc`. Defaults `grok-4.6-fast-xhigh` for code, `claude-fable-5-1-thinking-max` for prose and judgment). Code delegates tier by difficulty. The hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your strongest judgment model (`claude-fable-5-1-thinking-max`), whether the task needs judgment on vague intent or is a precisely specified sequence of steps to execute to the letter. Trivial mechanical edits go to your fast code model. Per-role lines in the `/setup-stuc` rule override these defaults and the model choices in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`). A role with no line keeps its default, and a role line of `inherit-parent` or `auto` runs that role on the parent chat model (omit Task `model`).
...
Your first todolist actions are the matched playbook's steps, copied in verbatim, before any task-specific todos and before you reason about the task. The failure mode is reading a playbook then writing a bespoke plan that drops its named steps (`architect`, the throughput checkpoint). A step you choose not to do stays in the list with a one-line `skip: <reason>`; skipping silently is not allowed. Copy-the-last-screen / smallest-diff is a valid skip reason for `architect` in this stack. Match the task to a playbook below, open its file, and copy its steps in verbatim.
