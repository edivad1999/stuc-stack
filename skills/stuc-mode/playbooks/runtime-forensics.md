...
1. Capture the live signal on the matching surface via **android-verify**: a Perfetto or Studio CPU profile for a spinning process, a heap dump for a leak, an `android screen` / layout capture for a visual glitch. A real artifact, not a guess. For unknown-path glitches, soak, or logcat-plus-keyframes, load **stuc-artemis** (Pro) on that serial, then keep the layout JSON, screen PNG, Perfetto, or heap as the smoking-gun artifact. An Artemis report is not that artifact. Do not call `control-ui` or `control-cli`.
