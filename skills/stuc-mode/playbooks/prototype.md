...
5. Verify on the matching surface. For a visual Android decision, screenshot each variant via **android-verify** and drive the interaction; the eye is the test. For a throwaway HTML sketch, capture the render yourself; do not call `control-ui`. For a behavioral or timing decision, observe the thing you are deciding by logging the timing, printing the output, or watching the render. The observation is the test here, not an assertion.
