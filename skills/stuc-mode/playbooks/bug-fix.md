...
1. Reproduce it yourself on the matching surface. Android: **android-verify** (Gradle assemble, `android describe` / `run` / `layout` or `screen`), then `docs/verify-<app>/` when it exists. If the tap path is unknown, cross-app, or blocked by a system dialog, load **stuc-artemis** first, then prove the same repro with android-verify. An Artemis trace is not the repro proof. Do not call `control-ui` or `control-cli`. Don't hand the repro to the user. A debug or instrumentation protocol that says to ask the user does not override this; you drive the instrumented runtime. Ask the user only with a stated, specific reason the surface cannot be reached, and only after driving it as far as it goes. Won't reproduce directly, force it. Synthesize the trigger, tighten conditions, or instrument until it fires. A bug you can't reproduce, you can't prove fixed.
...
3. Plan the fix. If it crosses a function boundary, `architect` first. Copy-the-last-screen / smallest-diff may skip with `architect skipped: <reason>`. Delegate implementation to a subagent using your configured bug-fix model (default `grok-4.6-fast-xhigh`) with a specific scope; review the diff.
