...
- `## Verification`. State how you ran each check and its rigor. Name the real path (`android-verify`, a named Gradle task, `docs/verify-<app>/`). A **stuc-artemis** trace is an exploration receipt, not this section's proof. State the outcome of each check, not only the command name. Do not claim checks that were not run. (P18)
