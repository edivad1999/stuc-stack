...
4. Verify each component against its baseline via image diff on the matching surface. Android: the project's named screenshot/preview Gradle task when it exists (blocking, P16); otherwise **android-verify** (`android screen capture` / Studio Compose preview). Do not use **stuc-artemis** for pixel diffs. Artemis keyframes are not a baseline. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. `/loop` per component until the diff is zero.
