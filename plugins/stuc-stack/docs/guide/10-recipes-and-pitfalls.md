...
```text
/swarm check every Android module that has unit tests. one worker per module. one report.
```

Each worker owns one module. The parent waits for every slice and returns one `PASS`, `ISSUES`, or `BLOCKED` report instead of raw worker dumps.
...
```text
/stuc-mode repro the duplicate write first. if there's a cheap test path, /tdd it. then fix and rerun.
```
