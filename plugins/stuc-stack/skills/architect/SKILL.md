...
## Skip (this stack only)

Copy-the-last-screen and smallest-diff work may skip this skill with `architect skipped: copy last similar screen` (or the real reason). That skip is an exception, not a rewrite of the phases below. When this skill runs, follow them as written: arena of structurally distinct candidates, `not implemented` bodies, design-it-twice.
