...
You are producing one candidate design in architect's parallel exploration. Read the **architect** skill in full first; that's the workflow you're inside. Output a candidate design package: type sketch, function signatures, module map, and prose rationale shaped per [`rationale-template.md`](rationale-template.md). Kotlin sealed types, Compose slots, and Gradle module maps are the usual illustrations in this stack; the discipline is the same as any typed language.
