...
1. Capture a baseline trace via the matching surface. Android UI jank: **android-verify** plus chrisbanes `compose-recomposition-performance` (via **using-chrisbanes-skills**). Studio/profiler workflows: android/skills `android-profiler` (via **using-android-skills**). Do not paste those upstream bodies. Other surfaces: the control skill in Non-negotiables.
