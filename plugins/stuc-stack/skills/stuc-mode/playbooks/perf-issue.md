...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Copy-the-last-screen / smallest-diff may skip with `architect skipped: <reason>`. Delegate implementation to a subagent using your configured perf-issue model (default `gpt-5.6-sol-max`); review the diff. Capture a post-fix trace.
