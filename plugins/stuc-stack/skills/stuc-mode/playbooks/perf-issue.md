...
1. Capture a baseline trace via the matching surface. Android UI jank: **android-verify** plus chrisbanes `compose-performance` (via **stuc-chrisbanes**, then the installed chrisbanes router **using-chrisbanes-skills**). Studio/profiler workflows: android/skills `android-profiler` (via **using-android-skills**). Do not paste those upstream bodies. Other surfaces: the control skill in Non-negotiables.
...
3. Plan the fix from the trace. Do not run `architect` because the change crosses a function. Run `architect` only for novel ownership/layering with no in-repo precedent. Delegate implementation to a subagent using your configured perf-issue model (default `gpt-5.6-sol-max`); review the diff. Capture a post-fix trace.
