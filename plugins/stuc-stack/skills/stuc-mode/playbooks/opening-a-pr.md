...
- `## Verification`. State how you ran each check and its rigor. Name the real path (`android-verify`, a named Gradle task, `control-cli`, `control-ui`). State the outcome of each check, not only the command name. Do not claim checks that were not run. (P18)
