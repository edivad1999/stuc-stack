...
Use this skill to write or review the shape of Kotlin branching code. Treat it as a refactoring **procedure**. There is no P#; do not add architecture opinions here.
...
- chrisbanes `kotlin-flow-state-event-modeling` and `kotlin-coroutines-structured-concurrency` (install chrisbanes/skills; do not copy) for Flow and coroutine primitive choices.
- chrisbanes `kotlin-multiplatform-expect-actual` for platform actuals staying thin.
- **kotlin-best-practices** in this plugin for type/API overlays.
