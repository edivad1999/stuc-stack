...
Open a todolist whose first item is to read the Principles section of the **stuc-mode** skill. Then add the phases below as todos.
...
- For one-way-door design decisions with **no in-repo precedent**, run the **architect** skill. Skip it for mechanical work, copy-the-last-screen, and smallest-diff tasks. Do not run arena of four models for a small change (the **laziness-protocol** principle skill).
