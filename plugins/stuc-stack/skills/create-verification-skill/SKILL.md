...
Every serious project needs a scripted way to drive the real app and prove behavior: launch it, exercise a feature the way a user would, and capture evidence. Android Drive defaults below are overlay (P18: do not claim coverage you did not run). This skill generates that as a project-local skill (`.cursor/skills/verify-<app>/`) tailored to the repo. You write the generator's output for the next agent, not for a human: it will be read cold, mid-task, by an agent that has never seen the app.
...
- **Drive:** how can an agent interact with it programmatically? Existing harnesses first. If the primary surface is an Android app, Drive **must** call the **android-verify** skill and the `android` binary (official `android-cli` skill), not Playwright, not `control-ui` / `control-cli`. Do not paste android-cli into the generated skill. For web/Electron, browser/CDP; for CLI/TUI, tmux/PTY; for services, HTTP. Interview the repo; non-Android primary surfaces keep the generic recipe.
