...
Applies to any typed language. Skills like `kotlin-best-practices` ground it in specific syntax.

**The patterns:**

- **Make illegal states unrepresentable.** Model variants as sum types: sealed classes / sealed interfaces in Kotlin, discriminated unions in TypeScript, enums with payloads in Rust/Swift, ADTs in Haskell/OCaml. Don't model state as a bag of optional fields where contradictory combinations compile. A subtle anti-pattern worth naming: `isComplete: Boolean` plus `completedAt: Instant?` admits `isComplete == true && completedAt == null`, which is meaningless. Derive the boolean from a single source like `completedAt != null`, or model the variants explicitly:
...
- **Brand semantic primitives.** `UserId` and `OrderId` are strings underneath but should not be interchangeable. Newtypes in Rust, opaque types in Swift, `@JvmInline value class` in Kotlin, phantom types in Haskell, branded intersections in TypeScript. Validate once at creation, trust the type downstream.
...
- **Don't lie to the type system.** Casts, `as`, `!!`, unsafe coercions, and assertion functions that bypass the compiler are runtime crashes waiting to happen. If the compiler can't prove a fact, prove it (validate, narrow, refine the model) or accept that the cast is a hazard. The cast you bury today is the postmortem you write next week.
- **Exhaustive matching is the compiler's job.** When you match on a sum type, the compiler must fail compilation if a new variant is added without handling. Use the idiom your language provides: `when` on a sealed type with no `else` in Kotlin, `never`-typed binding in TypeScript, unannotated `match` in Rust, `-Wincomplete-patterns` in Haskell.
